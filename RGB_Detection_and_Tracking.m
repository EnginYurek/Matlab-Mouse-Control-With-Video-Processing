...
greenThresh = 0.015; % Threshold for green detection
blueThresh = 0.07; % Threshold for blue detection
mouse =Robot();
...
while(nFrame < 2000 )
    ...
    if((centXGreen ~= 0) && (centYGreen ~= 0))
        mouse.mouseMove(centXGreen*(1366/480),centYGreen*(768/640));
        mouse.delay(20);
    end
    
    if((centXBlue ~= 0) && (centYBlue ~= 0) &&(centXRed == 0) && (centYRed == 0) &&(mod(nFrame,20)==0)) 
        mouse.mousePress(InputEvent.BUTTON1_MASK);  %sol týk
        mouse.mouseRelease(InputEvent.BUTTON1_MASK);
    end
    
    if((centXRed ~= 0) && (centYRed ~= 0) && (centXBlue == 0) && (centYBlue == 0) &&(mod(nFrame,20)==0) )
        mouse.mousePress(InputEvent.BUTTON3_MASK); %sað týk
